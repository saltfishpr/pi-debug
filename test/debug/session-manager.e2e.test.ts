...
describe.skipIf(!hasDelve)("DebugSessionManager e2e", () => {
  ...
  it("启动真实 Go 程序并完成断点检查与继续执行", async () => {
    ...
    expect(started).toMatchObject({
      execution: {
        kind: "stopped",
        thread: { stop: { event: { reason: "breakpoint" } } },
      },
    });
    ...
    expect(stack).toMatchObject({
      frames: { items: [{ index: 0, data: { name: "main.add", line: 6 } }] },
    });

    const evaluated = await session.evaluate({ frame: 0, expression: "a + b" });
    expect(evaluated).toMatchObject({ data: { result: "5" } });

    const resumed = await session.continue({ singleThread: false, waitMs: 10_000 });
    expect(resumed).toMatchObject({
      kind: "closed",
      status: { state: { state: "closed", reason: { kind: "terminated" } } },
    });
  }, 30_000);
});
