...
export class DebugSessionManager {
  ...
  private async dispatch(args: DebugArguments, cwd: string, signal: AbortSignal): Promise<unknown> {
    if (args.action === "configurations") return loadDebugConfigurations(cwd);
    if (args.action === "start") {
      ...
      const selected: DebugConfiguration | undefined =
    ...
    if (args.action === "stop") {
      await this.session?.close();
      const result = this.session?.status() ?? { state: "terminated" };
      this.session = undefined;
      return result;
    }
    if (args.action === "status") return this.session?.status() ?? { state: "idle" };
    const session = this.session;
    if (!session) throw new Error("No debug session. Use debug start first.");
    switch (args.action) {
      case "wait":
        await session.wait(args.waitMs ?? 1000, signal);
        return session.status();
      case "set_breakpoints":
        if (!args.file || !args.lines)
          throw new Error("set_breakpoints requires file and lines; [] clears that file's breakpoints.");
        return session.setBreakpoints(args.file, args.lines, signal);
      case "continue":
      case "next":
      case "pause":
        return session.control(args.action, args.threadId, args.waitMs, signal);
      case "step_in":
        return session.control("stepIn", args.threadId, args.waitMs, signal);
      case "step_out":
        return session.control("stepOut", args.threadId, args.waitMs, signal);
      case "stack_trace":
        return session.stackTrace(args.threadId, signal);
      case "variables":
        return session.variables(args.threadId, args.frame, args.scope, args.depth, args.maxChildren, signal);
      case "evaluate":
        return session.evaluate(args.expression, args.threadId, args.frame, signal);
      case "inspect":
        return {
          ...session.status(),
          ...(await session.stackTrace(args.threadId, signal)),
          ...(await session.variables(args.threadId, args.frame, args.scope, args.depth, args.maxChildren, signal)),
        };
    }
  }
}
